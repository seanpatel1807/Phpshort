                    ...
                    @foreach($groupSettings as $setting)
                        <div class="mb-4">
                            ...
                            @elseif($setting->name === 'custom_js')
                                <textarea id="{{ $setting->name }}" name="{{ $setting->name }}" class="w-full px-4 py-2 border border-gray-700 rounded-md bg-gray-700 text-black focus:outline-none focus:border-blue-500 text-lg" rows="4" placeholder="{{ $setting->payload }}">{{ $setting->payload }}</textarea>
                            
                            
                            @elseif(in_array($setting->name, ['logo', 'favicon']))
                                    <input type="file" id="{{ $setting->name }}" name="{{ $setting->name }}" class="w-full px-4 py-2 border border-gray-700 rounded-md bg-gray-700 text-black focus:outline-none focus:border-blue-500 text-lg">
                                @if ($errors->any())
